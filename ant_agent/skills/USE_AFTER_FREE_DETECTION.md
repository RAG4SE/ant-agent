...
4. **Hunt**: From the CFG, identify all **Use-After-Free** bugs. Record their positions and the reasoning behind classifying them as bugs. The bug identification process should follow a step-by-step pattern. For each bug candidate, **Verify** its correctness before identifying the next bug.
  - **What is Use-After-Free?**
    Use-after-free (UAF) is a memory-safety defect that arises when a program continues to access a heap object after it has been freed. Because the allocator may silently reuse that slot for a subsequent allocation, the stale pointer can be corrupted or hijacked, leading to crashes, information disclosure, or even full remote-code-exploitation. UAF is particularly dangerous in code paths that handle error cleanup, complex object lifetimes, or reference-counting mismatches, where the free may occur unexpectedly early and the remaining references remain seemingly valid. Detecting it therefore requires precise tracking of allocation/deallocation pairs and all subsequent dereferences across inter-procedural control flow.
   - **Verify**
